...
import { OpenRouterFoodParsingGateway } from "./openrouter-food-parsing.gateway";
...
test("requests structured JSON schema and parses a wrapped items response", async () => {
  ...
  const gateway = new OpenRouterFoodParsingGateway("test-api-key", "openai/gpt-oss-20b:free", {
    build: () => "meal prompt",
  });
