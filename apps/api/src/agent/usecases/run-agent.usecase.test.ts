...
describe("RunAgentUseCase — who may act on whom", () => {
  test("a user cannot act on another user's data", async () => {
    const { useCase, scripted } = setup();

    expect(await statusOfThrown(() => run(useCase, { userId: "user-2" }))).toBe(403);
    expect(scripted.lastInput).toBeUndefined();
  });

  test("a super admin acts on another user, and the writes belong to that user", async () => {
    const { useCase, notes } = setup({ calls: [{ name: "save_note", args: { content: "a" } }] });
    const admin = { userId: "admin", permissions: ["*:manage"], denies: [] };

    const response = await useCase.execute({ userId: "user-1", text: "pedido" }, admin);

    expect((await notes.findById(response.createdEntities[0].id))?.userId).toBe("user-1");
  });

  test("a super admin with any deny is refused", async () => {
    const { useCase } = setup();
    const admin = { userId: "admin", permissions: ["*:manage"], denies: ["event:delete"] };

    expect(await statusOfThrown(() => run(useCase, {}, admin))).toBe(403);
  });
});
