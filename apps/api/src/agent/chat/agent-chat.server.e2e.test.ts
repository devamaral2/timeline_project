...
import type { AgentChatServerFrame } from "@repo/entities/contracts";
...
beforeAll(async () => {
  fakeAuth = createServer((request, response) => {
    if (request.url !== "/auth/me") return void response.writeHead(404).end();
    ...
    response.end(JSON.stringify({ userId: user, roles: ["member"], permissions: [], denies: [] }));
  });
