...
it("derives a scoped non-reversible rate limit subject hash", () => {
  ...
  expect(rateLimitSubjectHash(key, "login_email", "a@example.test"))
    .not.toBe(rateLimitSubjectHash(key, "login_ip", "a@example.test"));
});
...
run("counts concurrent hits atomically and resets exactly at the window boundary", async () => {
  ...
  const hits = await Promise.all(Array.from({ length: 3 }, () => limiter.hit({ scope: "login_email", subject: "user@example.test", limit: 2, windowSeconds: 60, now: start })));
  expect(hits.filter(({ allowed }) => allowed)).toHaveLength(2);
  expect(hits.find(({ allowed }) => !allowed)?.retryAfterSeconds).toBe(60);
  await expect(limiter.hit({ scope: "login_email", subject: "user@example.test", limit: 2, windowSeconds: 60, now: new Date(start.getTime() + 60_000) })).resolves.toEqual({ allowed: true, retryAfterSeconds: 0 });
});
