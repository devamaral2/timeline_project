...
import { lockActiveSigningKey } from "../security/postgres-signing-key.repository";
import { buildUnsignedAccessTokenClaims, type SignAccessToken } from "../security/jwt";
...
import type { AuthenticatedActor, AuthenticationMethod, ResolvedAccess } from "../../domain/users/user";
...
export interface RevokeAllOfUserCommand { actor: AuthenticatedActor; now: Date; context: RequestContext; }
export interface FindActiveSessionQuery { sessionId: string; userId: string; }
...
export type SessionRepository = Pick<PostgresSessionRepository, 'rotateRefreshToken' | 'revokeByRefreshToken' | 'revokeAllOfUser' | 'findActiveSession'>;
