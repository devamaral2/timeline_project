...
describe('jwt', () => {
  ...
  it('nao serializa o clock interno como claim do token', () => {
    const unsignedClaims = buildUnsignedAccessTokenClaims({
      ...claimsAt(),
      now: NOW,
    });
    const token = signJwt(
      { ...unsignedClaims, jti: 'token-1' },
      { kid: key.kid, privateKey },
    );

    expect(unsignedClaims).not.toHaveProperty('now');
    expect(
      verifyJwt(token, keys, options.issuer, options.audience, NOW),
    ).toEqual({ ...unsignedClaims, jti: 'token-1' });
  });
