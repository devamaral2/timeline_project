...
describe("StartStepUpUseCase", () => {
  it("sends an OTP and stores the attempt bound to the caller session", async () => {
    ...
    const otp = { start: vi.fn().mockResolvedValue({ providerChallengeId: "provider", reportedChannel: "sms" }), check: vi.fn() };
    ...
    expect(result).toMatchObject({ purpose: "password_change", secondFactor: "otp", channel: "sms", maskedDestination: "+***********21" });
    expect(result.expiresAt).toEqual(new Date(now.getTime() + 10 * 60 * 1000));
    ...
    expect(command.challenge).toMatchObject({ providerChallengeId: "provider", requestedChannel: "sms", reportedChannel: "sms" });
  ...
  it("stores a channel mismatch already invalidated and reports the provider as unavailable", async () => {
    const startStepUpAttempt = vi.fn().mockResolvedValue("created");
    const otp = { start: vi.fn().mockResolvedValue({ providerChallengeId: "provider", reportedChannel: "whatsapp" }), check: vi.fn() };
    const usecase = new StartStepUpUseCase(users(), fakeAuthenticationRepository({ startStepUpAttempt }), otp, allowingLimiter, new FixedClock(), new FixedSecrets(), limits);
    ...
    const command = startStepUpAttempt.mock.calls[0]![0] as StartStepUpAttemptCommand;
    expect(command.invalidatedAt).toEqual(now);
    expect(command.challenge?.invalidatedAt).toEqual(now);
  });

  it("refuses a step-up for a user who is no longer active", async () => {
    const usecase = new StartStepUpUseCase(users({ ...activeUser, status: "suspended" }), fakeAuthenticationRepository(), { start: vi.fn(), check: vi.fn() }, allowingLimiter, new FixedClock(), new FixedSecrets(), limits);
