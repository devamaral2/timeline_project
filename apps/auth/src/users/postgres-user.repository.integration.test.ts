...
async function seedAdmin(database: AuthDatabase, id: string): Promise<void> {
  await database.query("INSERT INTO users(id,email,name,password_hash,status,created_at,updated_at) VALUES($1,$2,'Admin','hash','active',$3,$3)", [id, `${id.toLowerCase()}@example.test`, now]);
