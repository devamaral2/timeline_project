import { BadRequestException, Body, Controller, HttpCode, HttpStatus, Inject, Post, Req } from "@nestjs/common";
import { z } from "zod";
...
@Controller("auth") export class PublicAuthController {
  ...
  @Post("invites/inspect") async inspectInvite(@Body() body:unknown) { return this.inspect.execute(parse(inspectBody,body).token); }
  @Post("invites/accept") async accept(@Body() body:unknown,@Req() request:Request){const value=parse(acceptBody,body);return this.start.execute({inviteToken:value.token,password:value.password,phoneE164:value.phone,mfaChannel:value.channel,context:this.context(request)});}
  @Post("login") @HttpCode(HttpStatus.ACCEPTED) async startLogin(@Body() body:unknown,@Req() request:Request){const value=parse(loginBody,body);return this.login.execute({...value,context:this.context(request)});}
  @Post("mfa/verify") @HttpCode(HttpStatus.OK) async verify(@Body() body:unknown,@Req() request:Request){const value=parse(verifyBody,body);return this.verifyMfa.execute({mfaToken:value.mfaToken,code:value.code,context:this.context(request)});}
  @Post("mfa/recover") @HttpCode(HttpStatus.OK) async recover(@Body() body:unknown,@Req() request:Request){const value=parse(recoverBody,body);return this.complete.recover({...value,context:this.context(request)});}
  @Post("mfa/resend") @HttpCode(HttpStatus.ACCEPTED) async resendMfa(@Body() body:unknown){const value=parse(resendBody,body);return this.resend.execute(value.mfaToken);}
}
