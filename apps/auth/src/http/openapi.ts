...
const operations: Record<string, Record<string, OpenApiOperation>> = {
  ...
  "/auth/invites/accept": {
    post: { summary: "Inicia o aceite de convite", description: "Define a senha e o telefone do convidado e dispara o desafio MFA. A conta só é ativada depois da confirmação em `/auth/mfa/verify` ou `/auth/mfa/recover`.", tags: ["Convites públicos"], requestBody: request("AcceptInviteRequest"), responses: { "201": json({ $ref: "#/components/schemas/MfaChallenge" }), "422": json({ $ref: "#/components/schemas/ErrorCode" }, "Senha ou telefone não atende à política de segurança."), "503": json({ $ref: "#/components/schemas/ErrorCode" }, "O provedor de MFA não está disponível."), ...errorResponses } },
  },
  "/auth/login": {
    post: { summary: "Inicia login por senha", description: "Confere email e senha e inicia o segundo fator escolhido. O resultado é sempre um token temporário de MFA; credenciais incorretas não revelam se o email existe.", tags: ["Autenticação pública"], requestBody: request("LoginRequest"), responses: { "202": json({ $ref: "#/components/schemas/MfaChallenge" }), "503": json({ $ref: "#/components/schemas/ErrorCode" }, "O provedor de MFA não está disponível."), ...errorResponses } },
  },
  "/auth/mfa/verify": {
    post: { summary: "Confirma o código MFA", description: "Finaliza um login ou aceite de convite usando o código recebido. No sucesso, retorna tokens da sessão; no aceite de convite também retorna os códigos de recuperação, que devem ser guardados pelo usuário.", tags: ["Autenticação pública"], requestBody: request("VerifyMfaRequest"), responses: { "200": json({ $ref: "#/components/schemas/SessionTokens" }), "503": json({ $ref: "#/components/schemas/ErrorCode" }, "O provedor de MFA não está disponível."), ...errorResponses } },
  },
  "/auth/mfa/recover": {
    post: { summary: "Conclui MFA com código de recuperação", description: "Alternativa ao código por SMS/WhatsApp para finalizar um login ou aceite de convite. O código de recuperação é de uso único.", tags: ["Autenticação pública"], requestBody: request("RecoverMfaRequest"), responses: { "200": json({ $ref: "#/components/schemas/SessionTokens" }), ...errorResponses } },
  },
  "/auth/mfa/resend": {
    post: { summary: "Reenvia o desafio MFA", description: "Gera um novo desafio para uma tentativa MFA ainda válida, preservando o canal escolhido originalmente. Está sujeito a limite de tentativas.", tags: ["Autenticação pública"], requestBody: request("MfaTokenRequest"), responses: { "202": json({ $ref: "#/components/schemas/MfaChallenge" }), "503": json({ $ref: "#/components/schemas/ErrorCode" }, "O provedor de MFA não está disponível."), ...errorResponses } },
  },
  "/auth/token/refresh": {
...
export const authOpenApiDocument = {
  ...
  components: {
    ...
    schemas: {
      ...
      AcceptInviteRequest: object({ token: token("Token secreto presente no link de convite."), password: token("Senha inicial, validada pela política de segurança."), phone: string("Telefone E.164 usado no MFA (ex.: +5511999999999).", 64), channel: { type: "string", enum: ["sms", "whatsapp"], description: "Canal para envio do desafio." } }),
      ...
      VerifyMfaRequest: object({ mfaToken: token("Token temporário emitido ao iniciar login ou aceitar convite."), code: string("Código OTP recebido.", 64) }),
      RecoverMfaRequest: object({ mfaToken: token("Token temporário emitido ao iniciar login ou aceitar convite."), recoveryCode: token("Código de recuperação de uso único.") }),
      ...
      MfaChallenge: object({ mfaToken: string("Token temporário a enviar na confirmação MFA."), channel: { type: "string", enum: ["sms", "whatsapp"] }, maskedDestination: string("Destino mascarado do desafio."), expiresAt: dateTime("Expiração do desafio.") }, ["mfaToken", "expiresAt"]),
      SessionTokens: object({ accessToken: string("JWT para autenticar rotas protegidas."), refreshToken: string("Token opaco para renovar a sessão."), accessTokenExpiresInSeconds: { type: "integer", description: "Vida útil do access token em segundos." }, refreshTokenExpiresAt: dateTime("Expiração do refresh token."), recoveryCodes: { type: "array", description: "Códigos de recuperação, retornados apenas ao aceitar convite.", items: { type: "string" } } }, ["accessToken", "refreshToken"]),
      RefreshTokens: object({ accessToken: string("Novo JWT de acesso."), refreshToken: string("Novo refresh token; substitui o anterior.") }),
      StepUpChallenge: object({ stepUpToken: string("Token temporário a confirmar."), purpose: { type: "string", enum: ["password_change", "recovery_regeneration"] }, secondFactor: { type: "string", enum: ["otp", "recovery"] }, channel: { type: "string", enum: ["sms", "whatsapp"] }, maskedDestination: string("Destino mascarado do OTP."), expiresAt: dateTime("Expiração do desafio.") }, ["stepUpToken", "purpose", "secondFactor", "expiresAt"]),
