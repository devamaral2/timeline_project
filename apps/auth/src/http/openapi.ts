...
const operations: Record<string, Record<string, OpenApiOperation>> = {
  ...
  "/auth/mfa/verify": {
    post: { summary: "Confirma o código MFA", description: "Finaliza um login ou aceite de convite usando o código recebido. No sucesso, retorna tokens da sessão; no aceite de convite também retorna os códigos de recuperação, que devem ser guardados pelo usuário.", tags: ["Autenticação pública"], requestBody: request("VerifyMfaRequest"), responses: { "200": json({ $ref: "#/components/schemas/SessionTokens" }), "410": json({ $ref: "#/components/schemas/ErrorCode" }, "A MFA está temporariamente suspensa."), "503": json({ $ref: "#/components/schemas/ErrorCode" }, "O provedor de MFA não está disponível."), ...errorResponses } },
  },
  "/auth/mfa/recover": {
