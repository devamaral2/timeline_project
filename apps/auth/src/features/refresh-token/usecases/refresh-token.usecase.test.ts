...
import type { RotateRefreshTokenCommand, RotateRefreshTokenResult, SessionRepository } from "../../../auth-core/persistence/postgres-session.repository";
...
function repositoryReturning(
  ...
  it("rotates the refresh token and returns the new pair on success", async () => {
    ...
    const useCase = new RefreshTokenUseCase(repository, sign, clock, secrets);
  ...
  it("never invents a successor secret for the caller when the token was reused", async () => {
    ...
    const useCase = new RefreshTokenUseCase(repository, () => "unused", clock, secrets);
