...
import { normalizeEmail, type AuthenticatedActor } from "../../../domain/users/user";
 ...
 * perder, nao ha endpoint de reemissao nesta fase: so o hash do segredo fica
 * no banco. O operador precisa tratar a entrega como parte do fluxo externo.
 */
export class CreateInviteUseCase {
  ...
  async execute(input: CreateInviteInput): Promise<CreateInviteResult> {
    ...
    // Conflito seguro: a rota so chega aqui apos a autorizacao administrativa.
