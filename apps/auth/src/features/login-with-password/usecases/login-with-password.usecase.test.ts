...
import type { LoginCredentialChecker } from "../services/login-credential-checker";
...
function usecase(repoOverrides: object = {}) {
  return new LoginWithPasswordUseCase(users(), acceptingCredentials, allowingLimiter, fakeAuthenticationRepository(repoOverrides), new FixedClock(), new FixedSecrets(), limits, sign);
}
...
describe("LoginWithPasswordUseCase", () => {
  ...
  it("recusa credenciais inválidas antes de tocar o repositório", async () => {
    ...
    const login = new LoginWithPasswordUseCase(users(), rejectingCredentials, allowingLimiter, repo, new FixedClock(), new FixedSecrets(), limits, sign);
